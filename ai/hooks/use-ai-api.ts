...
import { GenAIClient } from "../lib/ai-client";
import { AILogEvent } from "../../types";
...
export type UseCoreAPIResults = {
  ...
  loggerStore: LoggerStore<AILogEvent>;
};

export function useAIAPI(): UseCoreAPIResults {
  ...
  const client = useMemo(() => new GenAIClient({ apiKey }), [apiKey]);
  ...
  const loggerStore = useMemo(() => createLoggerStore<AILogEvent>(), []);
  const logToStore = loggerStore.getState().log;

  useEffect(() => {
    const onLog = (log: AILogEvent) => {
      logToStore(log);
    };
