...
import { useAIAPIContext } from "../contexts/AIAPIContext";
...
import { coreFilters, AILoggerType } from "../lib/ai-log-filters";
...
type Message = { message: AILogEvent["message"]; filter?: AILoggerType; visibleElements?: ElementFilter; defaultExpanded?: boolean };
...
const component = (log: AILogEvent) => {
...
export default function AILogger({ filter = "all", logStyle = "console" }: LoggerProps) {
  const { loggerStore } = useAIAPIContext();
  const logs = useStore(loggerStore, (state) => state.logs as AILogEvent[]);
  ...
  const getComponent = (log: AILogEvent) => {
  ...
  const isLogVisible = (log: AILogEvent): boolean => {
